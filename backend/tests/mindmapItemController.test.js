...
describe("Mindmap Item Controllers", () => {
    ...
    describe("readMindmapItem", () => {
        ...
        it("should handle not found error", async () => {
            ...
            expect(res.json).toHaveBeenCalledWith({ message: "Mindmap item not found!" });
        });
    ...
    describe("updateMindmapItem", () => {
      it("should update an existing mindmap item", async () => {
          const req = { params: { id: 1 }, body: { content: "Updated Mindmap Item" } };
          const res = { json: jest.fn() };
          const mockMindmapItem = {
            id: 1,
            content: "Original Content",
              update: jest.fn().mockResolvedValue({ content: "Updated Mindmap Item" }),
          ...
          expect(res.json).toHaveBeenCalledWith({ id: 1, content: "Updated Mindmap Item" });
      });

      it("should handle not found error during update", async () => {
          const req = { params: { id: 999 }, body: { content: "Updated Mindmap Item" } };
    ...
    describe("deleteMindmapItem", () => {
        it("should delete an existing mindmap item", async () => {
            const req = { params: { id: 1 } };
            const res = { json: jest.fn() };
            const mockMindmapItem = { destroy: jest.fn().mockResolvedValue(undefined) };
            mindmap_item.findByPk.mockResolvedValue(mockMindmapItem);
            ...
            expect(res.json).toHaveBeenCalledWith({ message: "Mindmap item successfully deleted!" });
        });
