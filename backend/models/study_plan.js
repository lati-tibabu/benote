  ...
  module.exports = (sequelize, DataTypes) => {
    ...
    study_plan.beforeCreate(async (newPlan, options) => {
      ...
      const overlappingPlan = await study_plan.findOne({
        where: {
          ...
          [Op.or] : [
            { start_date: { [Op.between]: [newPlan.start_date, newPlan.end_date] } },
    ...
    study_plan.beforeUpdate(async (updatedPlan, options) => {
      const validDates = updatedPlan.start_date >= new Date().toISOString() && updatedPlan.end_date >= new Date().toISOString();
      ...
      const overlappingPlan = await study_plan.findOne({
        where: {
          ...
          [Op.or] : [
            { start_date: { [Op.between]: [updatedPlan.start_date, updatedPlan.end_date] } },
            { end_date: { [Op.between]: [updatedPlan.start_date, updatedPlan.end_date] } },
            {
