...
const giveUserMembership = async (req, res) => {
    try {
        ...
        if (!user_id && !team_id) {
            return res.status(400).json({ message: "Either user_id or team_id is required" });
        }
        ...
        const existingMembership = await workspace_membership.findOne({
            where: { workspace_id, user_id: user_id || null, team_id: team_id || null }
        });
    ...
    try {
        ...
        if(_workspace){
            ...
            if(is_member_and_admin){
                ...
                res.status(401).json({message: 'Unauthorized: You are not an admin of this workspace'});
            }
